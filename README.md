...
AI 出图 + 纯 HTML/CSS 合成的国风古诗展示页面。单文件、无依赖、浏览器打开即用。
...
- [一、原视频的逆向结论](#一原视频的逆向结论)
- [二、文件结构](#二文件结构)
- [三、如何修改成其他诗](#三如何修改成其他诗)
- [四、出图命令](#四出图命令)
- [五、渲染验证](#五渲染验证)
- [六、交互：诗句「春风拂柳」飘动](#六交互诗句春风拂柳飘动)
- [七、移动端适配](#七移动端适配)
- [八、线上部署](#八线上部署)
- [九、字体：简体宋体](#九字体简体宋体)
- [十、页面交互速查](#十页面交互速查)
...
原视频**不是"视频生成"，是一个网页应用的录屏演示**。

| 证据 | 说明 |
|---|---|
| 第 8 秒帧显示 `localhost:5173` | Vite 默认开发端口，博主在本地跑自建 Web 项目 |
| 多页面切换（黄鹤楼页 / 诗文化地图 / 唐诗三百首） | 单页应用路由 |
| 左下角真人小窗 + 底部字幕条 | 录屏 + 画中画，不是视频合成 |
| 插画风格 | AI 文生图产物，非 3D 渲染 |

**要复刻的"古诗画面"技术栈：**

- **插画**：AI 文生图，风格 = 国潮宣纸质感水墨插画（仿古宣纸颗粒底纹 + 淡墨远山 + 低饱和暖黄/赭石）
- **竖排诗**：CSS `writing-mode: vertical-rl` + `text-orientation: upright`
- **作者卡**：横排现代排版 + AI 生成作者古装画像
- **动态**：Ken Burns 缓慢推拉 + 逐列墨迹浮现
...
**只有 `index.html` + `assets/` 是运行必需的**，其余都是开发/验证工具。
...
```bash
export VOLCANO_API_KEY=<your-key>
cd poem-video

# 只出主插画
python3 gen_images.py --only main --n 2 --size 2048x2048

# 只出作者画像
python3 gen_images.py --only portrait --n 1 --size 2048x2048
...
**注意**：5.0 版本最小尺寸限制 **3686400 像素**（≈1920x1920），低于此值报 `InvalidParameter`。
...
发布脚本会：拷 `index.html` + `assets` + `favicon` → `/var/www/poem/`，
